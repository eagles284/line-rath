...
def schedule():
    ...
    thread = []
    for absen in absenList():
        thread.append(myThread(i, absen, i))
        thread[i].start()
        print('thread', i)
        i = i + 1
...
def scheduleAbsen(absen_id):
    ...
    if checkPerAbsen(absen_id):
        print('Absen id is existed. Not adding into schedule')
        return
    else:
        active_absens.append(absen_id)
        print('Absen id is not exist. Adding...')
